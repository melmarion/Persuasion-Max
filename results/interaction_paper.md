...
**Finding 7:** The technique × MFT matrix produces **0pp moral reframing lift** with heuristic extraction — all four conditions (care×liberal, care×conservative, loyalty×liberal, loyalty×conservative) score identically at 48.5% compliance. The heuristic regex extractor cannot differentiate care vs loyalty language at the appraisal level. The +8.4pp figure reported in the recipient test suite (test_recipient.py, liberal vs conservative with Ollama-detected techniques) reflects a different evaluation path using LLM extraction, not the MFT matrix. Moral reframing is architecturally supported but empirically blocked by the heuristic extraction ceiling. LLM extraction at scale is required to validate this capability.
